...
static int set_recv_socket_timeout( int fd,
                                    int timeout_ms )
{
    int err;
...
static int set_snd_socket_timeout( int fd,
                                   int timeout_ms )
{
    int err;
...
static int socket_sectag_set( int fd,
                              int sec_tag )
{
    int err;
    int verify;
...
static int socket_pdn_id_set( int fd,
                              uint8_t pdn_id )
{
    int err;
    char buf[ 8 ] = { 0 };
...
static int host_lookup( const char * host,
                        int family,
                        uint8_t pdn_id,
                        struct sockaddr * sa )
{
...
int socket_send( const struct download_client * client,
                 size_t len,
                 int timeout )
{
    int err;
...
static int error_evt_send( const struct download_client * dl,
                           int error )
{
...
void download_thread( void * client,
                      void * a,
                      void * b )
{
    int rc = 0;
